...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-green-900 flex justify-center items-center w-full flex-col mt-10">
      <div className="container py-8 ">
        <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-5 mx-28">
          {footerArr.map((item) => (
            <div key={item.key} className="mt-3 ">
              {item.image && (
                <div>
                  ...
                  <div className=" p-6 flex items-center justify-center mt-3">
                    <div className="container max-w-screen-lg mx-auto">
                      
                        <div className="flex flex-wrap justify-center gap-2">
                          <button className="bg-white p-2 font-semibold text-black inline-flex items-center space-x-2 rounded-full">
                  ...
                  <button className="bg-[#E4E5DF] hover:bg-slate-400 text-black font-semibold  py-2 px-4 text-center inline-flex items-center gap-1 mx-12 rounded-full">
                    Contact Us
                  </button>
                </div>
              )}
              {item.title && (
                <h2 className="mb-3 text-xl font-bold text-white ml-32">{item.title}</h2>
              )}
              <ul>
                {Object.values(item)
                  .filter((val, index) => index >= 2)
                  .map((listItem, index) => (
                    <li className="mt-3 text-white ml-32" key={index}>
                      {listItem}
                    </li>
                  ))}
              </ul>
      ...
      <div className="items-center justify-center w-full text-left ml-40 text-white">
